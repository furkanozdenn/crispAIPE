...
def load_model_and_data(config_path, checkpoint_path, model_class, variant_config_path=None):
    ...
    config_dir = os.path.dirname(os.path.abspath(config_to_use))
...
def create_ablation_table(results_dict, output_dir):
    ...
    if plt is None:
        print("matplotlib is not installed; skipping table and bar plot images.")
        return df
...
def main():
    ...
    if args.transformer_checkpoint and os.path.exists(args.transformer_checkpoint):
        ...
        transformer_config = os.path.join(os.path.dirname(args.config), 'crispAIPE_transformer_only_revs_conf.json')
    ...
    if args.cnn_checkpoint and os.path.exists(args.cnn_checkpoint):
        ...
        cnn_config = os.path.join(os.path.dirname(args.config), 'crispAIPE_cnn_only_revs_conf.json')
