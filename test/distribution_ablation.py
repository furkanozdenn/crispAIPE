...
def load_model_and_data(config_path, checkpoint_path, model_class, variant_config_path=None):
    ...
    # Fix data paths
    config_dir = os.path.dirname(os.path.abspath(variant_config_path if variant_config_path else config_path))
...
def evaluate_model(model, data_module, batch_size=64):
    ...
    for batch in tqdm(test_loader):
        ...
        with torch.no_grad():
            ...
            if isinstance(model, crispAIPE_Softmax):
            ...
            elif isinstance(model, crispAIPE):
...
def create_distribution_table(results_dict, output_dir):
    ...
    if plt is None:
        print("matplotlib is not installed; skipping table image.")
        return df
...
def main():
    ...
    if args.softmax_checkpoint and os.path.exists(args.softmax_checkpoint):
        ...
        print("="*60)
        softmax_config = os.path.join(os.path.dirname(args.config), 'crispAIPE_softmax_revs_conf.json')
    ...
    if args.logit_normal_checkpoint and os.path.exists(args.logit_normal_checkpoint):
        ...
        print("="*60)
        logit_config = os.path.join(os.path.dirname(args.config), 'crispAIPE_logit_normal_revs_conf.json')
