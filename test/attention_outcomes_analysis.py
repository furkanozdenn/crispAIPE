...
def parse_args():
    ...
    parser.add_argument(
        ...
        default='predicted',
        help='Efficiency values used for attention-efficiency correlations'
    )
...
def _mean_region_attention(position_attention, region_mask):
    if np.any(region_mask):
        return float(np.mean(position_attention[region_mask]))
    return np.nan
...
def _safe_corr(x, y):
    ...
    if np.sum(valid) < 3 or len(np.unique(x[valid])) < 2 or len(np.unique(y[valid])) < 2:
        return np.nan, np.nan, int(np.sum(valid))
    return pearsonr(x[valid], y[valid])[0], spearmanr(x[valid], y[valid])[0], int(np.sum(valid))


def _write_attention_tables(
    output_dir,
    attention_metrics,
    position_attention_by_sample,
    editing_efficiency,
    efficiency_source,
    locations,
    correlations,
    ...
    positions = np.arange(position_attention_by_sample.shape[1])

    detailed_specs = [
        ("Protospacer 5' (pos 10-19)", "protospacer", (10, 19), None),
        ("Protospacer 3' (pos 20-29)", "protospacer", (20, 29), None),
        ("PBS Early (pos 13-18)", "pbs", (13, 18), None),
        ("PBS Late (pos 19-25)", "pbs", (19, 25), None),
        ("RT Template <=35 bp", "rt", None, "short"),
        ("RT Template >35 bp", "rt", None, "long"),
    ]

    regional_rows = []
    for label, region, pos_range, rt_length_group in detailed_specs:
        metric_values = []
        sample_mask = np.ones(len(editing_efficiency), dtype=bool)
        if region == "rt":
            base_region_masks = (locations["rt_initial"] > 0.5) | (locations["rt_mutated"] > 0.5)
            rt_lengths = np.sum(base_region_masks, axis=1)
            if rt_length_group == "short":
                sample_mask = rt_lengths <= 35
            elif rt_length_group == "long":
                sample_mask = rt_lengths > 35
        else:
            base_region_masks = locations[region] > 0.5

        for i in range(len(editing_efficiency)):
            region_mask = base_region_masks[i].astype(bool).copy()
            if pos_range is not None:
                start, end = pos_range
                pos_mask = (positions >= start) & (positions <= end)
                region_mask &= pos_mask
            metric_values.append(_mean_region_attention(position_attention_by_sample[i], region_mask))

        pearson_corr, spearman_corr, n_valid = _safe_corr(
            editing_efficiency[sample_mask], np.asarray(metric_values)[sample_mask]
        )
        regional_rows.append({
            ...
            "mean_attention": np.nanmean(np.asarray(metric_values)[sample_mask]),
        })

    regional_df = pd.DataFrame(regional_rows)
    regional_df.to_csv(os.path.join(output_dir, "regional_attention_correlations.csv"), index=False)

    quartile_df = pd.DataFrame(quartile_stats)
    quartile_df.to_csv(os.path.join(output_dir, "attention_quartile_stats.csv"), index=False)

    metric_df = pd.DataFrame({
        "sample_index": np.arange(len(editing_efficiency)),
        "efficiency": editing_efficiency,
        "efficiency_source": efficiency_source,
        **{name: values for name, values in attention_metrics.items()},
    })
    metric_df.to_csv(os.path.join(output_dir, "per_sample_attention_metrics.csv"), index=False)

    position_df = pd.DataFrame({
        "position": positions,
        "mean_attention_received": np.mean(position_attention_by_sample, axis=0),
        "protospacer_mask_frequency": np.mean(locations["protospacer"], axis=0),
        "pbs_mask_frequency": np.mean(locations["pbs"], axis=0),
        "rt_initial_mask_frequency": np.mean(locations["rt_initial"], axis=0),
        "rt_mutated_mask_frequency": np.mean(locations["rt_mutated"], axis=0),
    })
    position_df.to_csv(os.path.join(output_dir, "position_wise_attention.csv"), index=False)

    summary = {
        "n_examples": int(len(editing_efficiency)),
        "efficiency_source": efficiency_source,
        "efficiency_bins": [float(x) for x in efficiency_bins],
        "pearson": {k: float(v) if np.isfinite(v) else None for k, v in correlations["pearson"].items()},
        "spearman": {k: float(v) if np.isfinite(v) else None for k, v in correlations["spearman"].items()},
        "regional_attention_correlations": regional_df.to_dict(orient="records"),
        "quartile_stats": quartile_stats,
        "attention_peak": {
            "position": int(position_df.loc[position_df["mean_attention_received"].idxmax(), "position"]),
            "mean_attention_received": float(position_df["mean_attention_received"].max()),
        },
    }
    with open(os.path.join(output_dir, "attention_reproduction_summary.json"), "w") as f:
        json.dump(summary, f, indent=2)

    return regional_df


def plot_attention_vs_efficiency(data, output_dir, efficiency_source='predicted'):
    ...
    if efficiency_source == 'ground_truth':
        editing_efficiency = ground_truth[:, 0]
    else:
        editing_efficiency = predictions[:, 0]
    ...
    for i, attn_matrix in enumerate(tqdm(attention_weights)):
        ...
        position_attention_by_sample.append(position_attention)
    ...
    position_attention_by_sample = np.vstack(position_attention_by_sample)
    ...
    regional_df = _write_attention_tables(
        output_dir,
        attention_metrics,
        position_attention_by_sample,
        editing_efficiency,
    ...
    plot_biological_insights_summary(correlations, quartile_stats, efficiency_bins, output_dir, regional_df)
...
def plot_biological_insights_summary(correlations, quartile_stats, efficiency_bins, output_dir, regional_df=None):
    ...
    if regional_df is not None:
        detailed_regions = [
            row["region"].replace(" (", "\n(").replace("RT Template", "RT")
            for _, row in regional_df.iterrows()
        ]
        detailed_corrs = [row["spearman_rho"] for _, row in regional_df.iterrows()]
    else:
        detailed_regions = [
            'Protospacer 5\'\n(pos 10-19)',
            'Protospacer 3\'\n(pos 20-29)',
        ...
        detailed_corrs = [
            correlations['spearman']['protospacer_attention'],
            correlations['spearman']['protospacer_attention'],
            correlations['spearman']['pbs_attention'],
            correlations['spearman']['pbs_attention'],
            correlations['spearman']['rt_initial_attention'],
            correlations['spearman']['rt_mutated_attention'],
        ]
    ...
    print(f"- regional_attention_correlations.csv: Directly computed sub-region correlations")
...
def main():
    ...
    plot_attention_vs_efficiency(data, args.output_dir, efficiency_source=args.efficiency_source)
