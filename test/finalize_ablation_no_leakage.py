"""
Finalize ablation study with correct train/test split (no data leakage).
This script waits for retraining to complete and then generates the final ablation table.
"""
...
def check_training_complete(log_dir, config_name, min_file_size=1000, wait_time=300):
    ...
    if checkpoint and os.path.exists(checkpoint):
        if os.path.getsize(checkpoint) > min_file_size:
            ...
            # If modified more than wait_time seconds ago, training is likely complete
...
def main():
    ...
    # Use the train_test_split config (correct split)
    base_config = os.path.join(project_root, 'pe_uncert_models/configs/crispAIPE_train_test_split_conf.json')
    ...
    # Hybrid checkpoint (from train_test_split config - correct split)
    hybrid_checkpoint = os.path.join(
        project_root,
        'pe_uncert_models/logs/crispAIPE_train_test_split_conf/2025-06-08-15-59-36/best_model-epoch=41-val_loss_val_loss=-3.0687.ckpt'
    )
    
    print("="*70)
    print("FINALIZING ABLATION STUDY (NO DATA LEAKAGE)")
    print("="*70)
    ...
    if not os.path.exists(hybrid_checkpoint):
        print(f"ERROR: Hybrid checkpoint not found: {hybrid_checkpoint}")
        return 1
    ...
    if hybrid_checkpoint and transformer_complete and cnn_complete:
        ...
        if result.returncode == 0:
            print("\n" + "="*70)
            print("ABLATION STUDY COMPLETE (NO DATA LEAKAGE)!")
            print("="*70)
